...
// function prototype for my encoding process
void dostuff(int); 
...
// my function to encode the information
...
void dostuff (int sock)
{
	...
	int i;
	int j;
	// list of acceptable input for encoding a file
    ...
    // create numerical version of coded message using modular math
	for(i = 0; i < size2-1; i++)
	{
		if((codedMessage[i] = (plaintext_codex[i] + cipher_codex[i]) % 27) > 27)
		{
    ...
    // create alpha version of coded message
	for(i = 0; i < size2-1; i++)
	{
		for(j = 0; j < 27; j++)
		{
    ...
    // write the coded message back to the client
